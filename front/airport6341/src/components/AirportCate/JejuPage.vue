    ...
    <div class="traffic">
      <div class="row">
        ...
        <div class="col-6">
          ...
          <el-descriptions direction="vertical" v-for="obj of state.parkinginfodata" :key="obj" border="1" style="width: 85%; margin: auto; height: 185px; margin-right: 100px;" >
...
export default {
    setup () {
      ...
      const handleData = async() => {
        ...
        // console.log(response3.data.items.item);
      ...
      const handleSearch = async(hno) => {
        router.push({path:'/board', query: { hno : [4,hno]}});
      };
      ...
      const handleData1 = async() => {
        const url = `/fligent/api/airportstore/selecttypeofbussiness.json?airportCode=CJU&typeOfBusiness=${state.typeofbusiness}&start=${state.page}`;
        const headers = {"Content-Type":"application/json"};
        const {data} = await axios.get(url, {headers});
        // console.log(data);
        if(data.status === 200){
